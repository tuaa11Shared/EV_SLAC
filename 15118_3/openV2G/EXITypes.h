...
#ifndef MAX_EXI_STREAM_LEN
#define MAX_EXI_STREAM_LEN 4096
#endif
...
/** \brief EXI Events */
	 ...
	 *
	 *	# General
	 *	.size defines the maximum size of the byte data_Array (see .data)
	 *
	 *	.data points to the input/output data_Array of bytes (unsigned char*).
	 *
	 ...
	 *	 byte needs to be read from the input stream/data-data_Array to the current byte buffer.
	 *
	 *	# Sending data (output)
	 *	.capacity is used for addressing single bits in the actual byte (see .buffer)
	 *	 and has to be set to 8, which means there are still 8 bits left to fill up
	 *	 the current byte buffer before writing the final byte to the output stream/data-data_Array.
	 *
	 */
...
#define FILE_STREAM 2
	...
	typedef struct {
		...
		uint32_t size;
		/**	byte data_Array data */
		uint8_t data[4096];
		/**	byte data_Array next position in data_Array */
		uint32_t pos;
#endif
#if EXI_STREAM == FILE_STREAM
		/** file pointer */
	...
	typedef struct {
		/** bytes data_Array size */
		size_t size;
		/** bytes data_Array data container */
		uint8_t* data;
		/** bytes data_Array length (len <= size) */
		size_t len;
	} exi_bytes_t;
	...
	typedef struct exiNameTablePrepopulated {
		/* number of namespaces AND length name-partitions data_Array */
		size_t len;
...
#define EXI_MAXIMUM_NUMBER_OF_NAME_PARTITION_ENTRIES 25
	...
	typedef struct {
		...
		struct entry_exi_name_partition_t {
...
/* Value string table */
	...
	typedef struct exiValueStringTable {
		...
		size_t size;
		/** string table entry data_Array container */
