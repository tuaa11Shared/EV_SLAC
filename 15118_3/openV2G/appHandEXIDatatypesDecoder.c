					...
					switch (eventCode) {
						case 0:
							if (appHandAnonType_supportedAppProtocolReq->AppProtocol.arrayLen < appHandAnonType_supportedAppProtocolReq_AppProtocol_ARRAY_SIZE) {
								errn = decode_appHandAppProtocolType(stream, &appHandAnonType_supportedAppProtocolReq->AppProtocol.data_Array[appHandAnonType_supportedAppProtocolReq->AppProtocol.arrayLen++]);
							}
							else {
						...
						case 0:
							...
							if (errn == 0) {
								if (eventCode == 0) {
									...
									appHandAnonType_supportedAppProtocolRes->ResponseCode = (appHandresponeType)uint32;
								}
								else {
									/* Second level event (e.g., xsi:type, xsi:nil, ...) */
									errn = EXI_UNSUPPORTED_EVENT_CODE_CHARACTERISTICS;
